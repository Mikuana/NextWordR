...
```{r backoff}
library(edgeR)
keygroups = select(df2, key1, key2) %>% unique
keygroups = keygroups[30000,]
apply(keygroups, 1, function(x) {
  sugg = 
    df2 %>% 
    filter(key1 == x[['key1']] & is.na(key2)) %>% # subset keygroup records
    mutate(gtprop = goodTuringProportions(freq))  # calculate proportions for seen records

  P0 = goodTuring(sugg$freq)$P0                   # calculate prevalence of unseen grams

  sugg_drop = 
    df1 %>%                                       # switch to lower level ngram
    filter(is.na(key1) & is.na(key2)) %>%         # subset drop level keygroup records
    mutate(gtprop = goodTuringProportions(freq))  # calculate proportions of seen records

  dropCoverP =
    sugg_drop %>%
    filter(suggest %in% sugg$suggest) %>%         # only suggestions covered by higher ngram
    .$gtprop %>% sum                              # calculate total covered proportion
  
  sugg_drop = 
    sugg_drop %>%
    filter(!suggest %in% sugg$suggest) %>%        # remove suggestions already in higher ngram
    mutate(
      gtprop = gtprop / (1 - dropCoverP),         # rescale prop after removing covered ngrams
      gtprop = gtprop * P0,                       # rescale prop as share of P0
      key1 = x[['key1']],                         # set keys of lower level to match higher
      key2 = x[['key2']]
    )

  bind_rows(sugg, sugg_drop) %>%        # bind lower level ngram with higher
    arrange(desc(gtprop))
})
